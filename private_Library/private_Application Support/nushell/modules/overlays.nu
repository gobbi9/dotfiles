...
def overlay_exports [module_path: string] {
...

  {
    ...
    exported: { commands: $commands, aliases: $aliases, externs: $externs }
  }
}

def format_inline_list [items: list<string>] {
  if ($items | is-empty) { "[]" } else { $"[($items | str join ', ')]" }
}

def print_overlay_exports [module_path_display: string, exports: record] {
  ...
  print $"(ansi yellow)externs:(ansi reset) (ansi green)(format_inline_list $exports.exported.externs)(ansi reset)"
  print ""
  print $"(ansi steelblue1a)  Type help <command | alias | extern> for more info.(ansi reset)"
}

def print_overlay_inactive_hint [] {
  print $"(ansi orangered1)  Overlay not active  (ansi reset) Activate it from repo root with (ansi green_bold)o(ansi reset)."
  print ""
}

def resolve_overlay_module_path [cwd?: string, module_path: string = "overlay.nu"] {
...

  {
    ...
    module_path_display: $module_path_display
  }
}

def overlay_is_active [resolved_module_path: string, exports: record] {
  let exported_names = (
  ...
  | any {|name|
      ...
      if ($resolved | is-empty) {
        ...
        $resolved | any {|entry| (($entry.path? | default "") == $resolved_module_path) }
      }
    }
}
...
export def "i" [
  ...
  let path_info = (resolve_overlay_module_path $cwd $module_path)
  let resolved_module_path = $path_info.resolved_module_path
  let module_path_display = $path_info.module_path_display

  if not ($resolved_module_path | path exists) {
    error make --unspanned { msg: $"Overlay module not found: ($resolved_module_path)" }
  }

  let exports = (overlay_exports $resolved_module_path)
  let is_active = (overlay_is_active $resolved_module_path $exports)

  if not $is_active {
    print_overlay_inactive_hint
  }

  print_overlay_exports $module_path_display $exports
